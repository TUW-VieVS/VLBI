...
simHeader = {'n_sim','dUT1_[mus]','x_pol_[muas]','y_pol_[muas]','x_nut_[muas]','y_nut_[muas]', 'average_3d_coordinates_[mm]', simHeader{7:end}};

mean_sig = t_mean_sig.Variables;
average = mean(mean_sig(:,7:end),2);
mean_sig = [mean_sig(:,1:6), average, mean_sig(:,7:end)];

rep = t_rep.Variables;
average = mean(rep(:,7:end),2);
rep = [rep(:,1:6), average, rep(:,7:end)];
