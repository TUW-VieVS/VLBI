...
function [opt] = lsmopt(antenna, sources, na, ns_q, ns_s, obs_per_source, obs_per_satellite, parameter, opt)
    ...
    opt.addDatumCd = opt.addDatumCd;         % 1 datum contraints are applied
                                             % 0 datum contraints are not applied 
    ...
    opt.pw_stc = opt.pw_stc;      % 0 estimate all selected stations as one offset per session (NNT/NNR is available)
        ...
        opt.nns_stc = opt.nns_stc;        % 1 Scale for stations 
    ...
    opt.SatPos.pw_sat = opt.SatPos.pw_sat;      % 1 Estimate satellite coordinates as PWL offsets, flag
                                  % 0 Do not estimate satellite coordinates as PWL offsets, flag
    
    % Estimation interval for PWL offsets [minutes]                                    
    opt.SatPos.sat_pos_int     = opt.SatPos.sat_pos_int; 
        
        opt.SatPos.constr_sat = opt.SatPos.constr_sat;    % 1 constraints between pwl source coordinates offsets, flag 
                                            % 0 no constraints between pwl, flag
    
        % Relative constraint between PWL offsets [cm]
        opt.SatPos.sat_pos_coef    = opt.SatPos.sat_pos_coef;  
    ...
    % Exclude additional stations from datum
    data='';
    if isfield(opt,'trf_excldatum')
        if opt.trf_excldatum
            stationsExclDatumFile = opt.trf_excldatum_file;
            fprintf(['Loading ' stationsExclDatumFile ' \n'])
            fid=fopen(['../TRF/SELECTION/STATNOTDATUM/' stationsExclDatumFile]);
            ...
            for i = 1:size(data{1},1)
                ...
                if sum(ids)>0    
                    ...
                    opt.stat(ids).nns_inc = 0;
    
                    opt.stat(ids).xyz_inc = 1;
                    opt.stc = 1;
                    opt.pw_stc = 0;
                    disp([antenna(ids).name,' is excluded from datum or is not fixed - listed in ' stationsExclDatumFile]);
                end
            end
        end
    end
    ...
    % ######## if specific stations should be used for NNR NNT ####
    
    %exclSta = [HART15M; HOBART12; ISHIOKA; KOKEE12M; MACGO12M; NYALE13S; RAEGYEB; WESTFORD; WETTZ13S];
    % exclSta = {'HART15M '; 'HOBART12'; 'KOKEE12M'; 'NYALE13S'};
    % 
    % if opt.stc ==1 
    %     for ista = 1:na
    %         if opt.nnt_stc == 1
    %             opt.stat(ista).nnt_inc = 1;
    %         else
    %             opt.stat(ista).nnt_inc = 0;
    %         end
    %         if opt.nnr_stc == 1
    %             opt.stat(ista).nnr_inc = 1;
    %         else
    %             opt.stat(ista).nnr_inc = 0;
    %         end
    %         if opt.nns_stc == 1
    %             opt.stat(ista).nns_inc = 1;
    %         else
    %             opt.stat(ista).nns_inc = 0;
    %         end
    %         
    %         for i=1:length(exclSta)
    %             if strcmp(exclSta(i),antenna(ista).name) 
    %                 if opt.nnt_stc == 1
    %                     opt.stat(ista).nnt_inc = 0;
    %                 end
    %                 if opt.nnr_stc == 1
    ...
    if ns_q ~= 0
        ...
        % Always estimate these sources (PWLO option)
        if opt.pw_sou
            if isfield(opt,'pw_sou_select')
                if strcmp(opt.pw_sou_select,'notcat_and_file') | strcmp(opt.pw_sou_select,'file')
                    if ~isempty(opt.pw_sou_select_file)
                        ...
                        for i = 1:size(data{1},1)
                            ids=[];
                            if strcmp(opt.pw_sou_select_file_sounames,'ivs')
                                ids = strcmp(deblank({opt.source.name}),deblank(data{1}(i,:))); % IVS in NNT/NNR    
                            elseif strcmp(opt.pw_sou_select_file_sounames,'iers')
                                ids = strcmp(deblank({opt.source.IERSname}),deblank(data{1}(i,:))); % IERS in NNT/NNR  
                            end
                            if sum(ids)>0
        ...
        % Specify sources for NNR in a text file
        data='';
        if opt.est_sourceNNR
            if isfield(opt,'est_sourceNNR_selection')
                if opt.est_sourceNNR_selection   
                    path2sourcelist = ['../CRF/SELECTION/SOUDATUM/' opt.NNR_sou_select_file];     %IVS names
                    fid = fopen(path2sourcelist);
                    if fid == -1
                        warning([path2sourcelist 'File with list of removed sources can not be found\n']);
                    else
                        data = textscan(fid, '%8s','Delimiter','\n','CommentStyle','#');
                        [opt.source.nnr_inc]=deal(0); % no source in NNR
                    end
                    fclose(fid);
            
                    for i = 1:size(data{1},1)
                        ids=[];
                        if strcmp(opt.NNR_sou_select_file_sounames,'ivs')
                            ids = strcmp(deblank({opt.source.name}),deblank(data{1}(i,:))); % IVS in NNT/NNR    
                        elseif strcmp(opt.NNR_sou_select_file_sounames,'iers')
                            ids = strcmp(deblank({opt.source.IERSname}),deblank(data{1}(i,:))); % IERS in NNT/NNR  
                        end
                        if sum(ids)>0    
                            opt.source(ids).nnr_inc = 1;
                        end
                    end
                end
            end            
    ...
    if ns_s ~= 0
        ...
        for isou = 1 : ns_s
            ...
            opt.satellite(isou).SatPos.sat_pos_coef     = opt.SatPos.sat_pos_coef;                         % satellite coordinate constratins [cm]
