...
% 2025-01-29, H. Wolf: - changed str2num to str2double 
%                      - chaned % to %% for faster performance
%                      -  added end to function
%                      -  changed code to allow a white noise of 0 for
%                         quasar and satellite osbervations
...
function saveSimParameters(hObject, handles)
    ...
    if get(handles.radiobutton_vie_sim_fromParameterFile,'Value') == 1
        ...
        if isreal(str2double(get(handles.edit_vie_sim_nDaysSim,'String'))) && str2double(get(handles.edit_vie_sim_nDaysSim,'String')) > 0
            simparam.idays = str2double(get(handles.edit_vie_sim_nDaysSim,'String'));
        else
            err = err + 1;
        end
        if isreal(str2double(get(handles.edit_vie_sim_startInd,'String'))) && str2double(get(handles.edit_vie_sim_startInd,'String')) > 0
            simparam.sind = str2double(get(handles.edit_vie_sim_startInd,'String'));
        else
            err = err + 1;
        end
    %     if err == 0
    ...
    elseif get(handles.radiobutton_vie_sim_specifyNow,'Value') == get(handles.radiobutton_vie_sim_specifyNow,'Max')
        ...
        if get(handles.checkbox_vie_sim_simSwd,'Value') == get(handles.checkbox_vie_sim_simSwd,'Max')
            if isreal(str2double(get(handles.edit_vie_sim_tropo_cn,'String'))) && str2double(get(handles.edit_vie_sim_tropo_cn,'String')) > 0
                simparam.Cn    = str2double(get(handles.edit_vie_sim_tropo_cn,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_H,'String'))) && str2double(get(handles.edit_vie_sim_tropo_H,'String')) > 0
                simparam.H     = str2double(get(handles.edit_vie_sim_tropo_H,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_wzd0,'String'))) && str2double(get(handles.edit_vie_sim_tropo_wzd0,'String')) > 0
                simparam.wzd0  = str2double(get(handles.edit_vie_sim_tropo_wzd0,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_vn,'String')))
                simparam.vn    = str2double(get(handles.edit_vie_sim_tropo_vn,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_ve,'String')))
                simparam.ve    = str2double(get(handles.edit_vie_sim_tropo_ve,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_dhseg,'String'))) && str2double(get(handles.edit_vie_sim_tropo_dhseg,'String')) > 0
            simparam.dhseg = str2double(get(handles.edit_vie_sim_tropo_dhseg,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_tropo_dh,'String'))) && str2double(get(handles.edit_vie_sim_tropo_dh,'String')) > 0
                simparam.dh    = str2double(get(handles.edit_vie_sim_tropo_dh,'String'));
            else
                err = err + 1;
            end
        end
        if get(handles.checkbox_vie_sim_simClock,'Value') == get(handles.checkbox_vie_sim_simClock,'Max')
            if isreal(str2double(get(handles.edit_vie_sim_clock_asd,'String'))) && str2double(get(handles.edit_vie_sim_clock_asd,'String')) > 0
                simparam.sy1   = str2double(get(handles.edit_vie_sim_clock_asd,'String'));
            else
                err = err + 1;
            end
            if isreal(str2double(get(handles.edit_vie_sim_clock_asdInt,'String'))) && str2double(get(handles.edit_vie_sim_clock_asdInt,'String')) > 0
                simparam.sy2   = str2double(get(handles.edit_vie_sim_clock_asdInt,'String'));
            else
                err = err + 1;
            end
        end
        if get(handles.checkbox_vie_sim_simNoise,'Value') == get(handles.checkbox_vie_sim_simNoise,'Max')
            ...
            if isreal(str2double(get(handles.edit_vie_sim_noise,'String'))) && str2double(get(handles.edit_vie_sim_noise,'String')) >= 0
                simparam.wn    = str2double(get(handles.edit_vie_sim_noise,'String'));
            else
                err = err + 1;
            end
    ...
    if simparam.rng_use_seed
        % Check input:
        [rng_seed, status] = str2num(get(handles.edit_vie_sim_rng_seed,'String'));
        if status
            if (rng_seed <= 0) || (mod(rng_seed, 1) ~= 0) 
                error('The seed for the random number generator defined in the VIE_SIM GUI has to be a nonnegative integer! Current seed: %f\n', wn_sat);
            else
                simparam.rng_seed = rng_seed;
            end
        else
