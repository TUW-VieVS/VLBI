% This function reads source coordintes from RFC and puts it values
% to a supersource CRF file.



function crf=manualRFCCrfToSupersouCrf(crf,manualCrfData)

    
% for all lines
for iLine=1:size(manualCrfData{1},1)
    % try to find corresponding souion in supersou TRF
     curSouInSupersouLog=strcmp({crf.IVSname}, manualCrfData{2}(iLine,1:8));
    ...
    if ~isempty(souIndInSupersou)
        ...
        %if strcmp('D', manualCrfData{2}(iLine,end))
            crf(curInd).rfc2024d.defining = 1;
        %else
        %    crf(curInd).manualCrf.defining = 0;
        %end
        crf(curInd).rfc2024d.ra = RA; %[rad]
        crf(curInd).rfc2024d.de = DE; %[rad]
        crf(curInd).rfc2024d.ra_sigma = deg2rad(manualCrfData{9}(iLine)/3600); %[rad]
        crf(curInd).rfc2024d.de_sigma = deg2rad(manualCrfData{10}(iLine)/3600); %[rad]
        crf(curInd).rfc2024d.corr = manualCrfData{11}(iLine); 
        ...
        crf(curInd).rfc2024d.numberSess = manualCrfData{14}(iLine); 
        crf(curInd).rfc2024d.numberObs = manualCrfData{12}(iLine); 
