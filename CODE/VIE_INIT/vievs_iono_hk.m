...
function [iono_x_corr_own, sigma_iono_x_corr_own, qflag_ion_own] = vievs_iono_hk(out_struct,ctau_s,ctau_x,csigma_tau_s,csigma_tau_x,parameter)
fileoutput=0;

tau_s=[ctau_s{:}]';
 tau_x=[ctau_x{:}]';
 sigma_tau_s=[csigma_tau_s{:}]';
 sigma_tau_x=[csigma_tau_x{:}]';

         iono_x_corr_own = 0;
         sigma_iono_x_corr_own = 0;
         qflag_ion_own = zeros(length(tau_x), 1);
...
%try % load all the variables from the vgosdb
   emptyCells = cellfun(@(x) strcmp(x, {''}), deblank(num2cell(out_struct.Observables.QualityCode_bX.QualityCode.val)));
   out_struct.Observables.QualityCode_bX.QualityCode.val(emptyCells)=['0'];
    QualityCode_X = str2num(out_struct.Observables.QualityCode_bX.QualityCode.val);% Quality Code Flag
    
    if fileoutput
        if ~isfield(out_struct.Observables.ChannelInfo_bX,'ChanAmpPhase')
    ...
    if fileoutput
        if ~isfield(out_struct.Observables.ChannelInfo_bX,'NumSamples')
    ...
    Repeat_X = {out_struct.Observables.ChannelInfo_bX.ChannelFreq.attr.name}; % check if channel frequency is same for all observations
   ...
   emptyCells = cellfun(@(x) strcmp(x, {''}), deblank(num2cell((out_struct.Observables.QualityCode_bS.QualityCode.val))));
   out_struct.Observables.QualityCode_bS.QualityCode.val(emptyCells)=['0'];

    QualityCode_S = str2num(out_struct.Observables.QualityCode_bS.QualityCode.val); % Quality Code Flag
    ...
    Repeat_S = {out_struct.Observables.ChannelInfo_bS.ChannelFreq.attr.name}; % check if channel frequency is same for all observations
