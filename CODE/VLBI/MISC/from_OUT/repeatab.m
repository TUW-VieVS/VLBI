...
if ~isempty(outfile)
    ...
    fprintf(fid,['# Baseline length repeatability from Vienna VLBI Software\n',...
        ...
        '# col4 (cols 51-56)  weighted baseline length repeatability in cm\n#\n'],...
        curClock(3:-1:1), curClock(4:6), limitation,...
        sum(~isnan(blr) & blr~=0));
    
    for iB=1:length(bas)
        if ~isnan(blr(iB)) && blr(iB)~=0 % if there was more than one estimate
            fprintf(fid, '%-17s %10.4f %13.4f %6.2f %6.2f\n',...
                bas(iB).name, mjd(iB), bl(iB), blr(iB)*100, wblr(iB)*100);
        end
    end    
    fclose(fid);    
end
...
if printToCommand==1
    fprintf(' nr baselineName----- basLength--- BLR---- weightd\n');
