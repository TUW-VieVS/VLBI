...
% 
%  ADAPTED for use eith TLE_propagation.m, Sep. 7, 2013 by Andreas
%  Hellerschmied
%
...
%     startmfe    - start time in minutes from epoch [min]
%     stopmfe     - stop time in minutes from epoch [min]
%     deltamin    - time step for propagation [min]
%
% 
%   coupling      :
%     getgravconst
...
function [satrec, startmfe, stopmfe, deltamin] = twoline2rv(whichconst, longstr1, ...
          longstr2, jdstart, jdstop, deltamin_in, ver_mode)
...
%     // fixes for bad input data values (missing, ...)
    ...
    % parse first line                              % H:
    carnumb = str2num(longstr1(1));                 % Line number
    satrec.satnum = str2num(longstr1(3:7));         % Sat. Number
    classification = longstr1(8);                   % Classification
    intldesg = longstr1(10:17);                     % International Designator
    satrec.epochyr = str2num(longstr1(19:20));      % Epoch Year
    satrec.epochdays = str2num(longstr1(21:32));    % Epoch Day
    satrec.ndot = str2num(longstr1(34:43));         % n'
    satrec.nddot = str2num(longstr1(44:50));        % n''
    nexp = str2num(longstr1(51:52));                % exponent of n''
    satrec.bstar = str2num(longstr1(53:59));        % Bstar, drag term 
    ibexp = str2num(longstr1(60:61));               % Bstar Exponent
    numb = str2num(longstr1(63));                   % Ephemeris type
    elnum = str2num(longstr1(65:68));               % Element number + check sum
 
    % parse second line
    if (ver_mode)                             
        cardnumb = str2num(longstr2(1));            % Line number
        satrec.satnum = str2num(longstr2(3:7));     % Sat number
        satrec.inclo = str2num(longstr2(8:16));     % i
        satrec.nodeo = str2num(longstr2(17:25));    % RA
        satrec.ecco = str2num(longstr2(26:33));     % e
        satrec.argpo = str2num(longstr2(34:42));    % omeg
        satrec.mo = str2num(longstr2(43:51));       % M
        satrec.no = str2num(longstr2(52:63));       % n
        revnum = str2num(longstr2(64:68));          % revolution numer + check sum
...
%     // ------ correct fix will occur when year is 4-digit in 2le ------
     ...
     % H: 
     if (~ver_mode)
         startmfe = (jdstart - satrec.jdsatepoch) * 1440.0;   % H: 1440=24*60, Minues from epoch [min]
         stopmfe  = (jdstop - satrec.jdsatepoch) * 1440.0;
         deltamin = deltamin_in;
     end;
...
% 
%                %[mon,day,hr,minute,sec] = days2mdhms ( startyear,startdayofyr);  %H; ????? NICHT days2mdh.m ??, zuvor:
%                [mon,day,hr,minute,sec] = days2mdh ( startyear,startdayofyr);
%                jdstart = jday( startyear,mon,day,hr,minute,sec);                 
%                %[mon,day,hr,minute,sec] = days2mdhms ( stopyear,stopdayofyr);    %H; ????? NICHT days2mdh.m ??
...
%      if (typerun == 'c')
%          startmfe =  -1440.0;   % H: + 1 day
%          stopmfe  =  1440.0;    % H: - 1 day
%          deltamin = 20.0;       % H: 20 min steps
%      end;
