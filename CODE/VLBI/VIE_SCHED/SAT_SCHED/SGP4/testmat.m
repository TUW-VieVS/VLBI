...
%   H: initl.m :    (a) calculation of GST with gstime.m, 
...
%        // ----------------- test simple propagation -------------------
    while (~feof(infile))
        ...
        if (feof(infile) == 0) % Nicht Ende der Datei!
...
%                // includes initialization of sgp4
            ...
            fprintf(1,' %d\n\n', satrec.satnum);    % H: Print to cCommand Window!
...
%                // loop to perform the propagation
            while ((tsince < stopmfe) && (satrec.error == 0))
                ...
                if (satrec.error == 0)
                    if ((typerun ~= 'v') && (typerun ~= 'c'))   % H: manaul mode
