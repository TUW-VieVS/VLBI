% Purpose  
%   Calculate the schedule (tag-along mode for special station) in STAR mode.
% History  
%   2016-05-25   Matthias Schartner and Lucia Plank:  Created
%   


function [sched] = vie_sched_tagalong_star(station1, source, obsmode, sched, PARA)

%%% find Station for STAR mode
strongantidx = find(strcmp(PARA.STRONGANT,{station1.name}));  
station2 = station1(strongantidx);
...
for isched = 1 : length(sched)
    ...
    % no fill-in for observation before a STAR source
    try
        if (source(sched(isched+1).scan(1).srcid).star==1 || ...
            source(sched(isched+2).scan(1).srcid).star==1 || ...
            source(sched(isched+3).scan(1).srcid).star==1)
            continue;
        end
    catch
    end
    ...
    for iscan = 1 : sched(isched).nscan
        ...
        for itag = 1 : stanum
            ...
            if (startmjd > sched(isched).scan(iscan).startmjd)
                if source(sched(isched).scan(iscan).srcid).star==1
                    warning('cannot reach STAR source in time!');
                end
                continue;
            end
...
%             end
            ...
            if (duration > maxscanlength && source(sched(isched).scan(iscan).srcid).star==0)
                continue;
            end
            
            strongant = find([sched(isched).scan(iscan).sta.staid]==strongantidx,1);
            if isempty(strongant)
                % add station to sched structure
                sched(isched).scan(iscan).nsta = sched(isched).scan(iscan).nsta + 1;
                sched(isched).scan(iscan).sta(sched(isched).scan(iscan).nsta).staid = strongantidx; %%%
                ...
                staobs(itag).ifirst = 0;
                staobs(itag).endmjd = sched(isched).scan(iscan).sta(strongant).endmjd;
                staobs(itag).az     = sched(isched).scan(iscan).sta(strongant).az;
                staobs(itag).el     = sched(isched).scan(iscan).sta(strongant).el;
                staobs(itag).ha     = sched(isched).scan(iscan).sta(strongant).ha;
                staobs(itag).dc     = sched(isched).scan(iscan).sta(strongant).dc;    
            end
        end    
    end
end
