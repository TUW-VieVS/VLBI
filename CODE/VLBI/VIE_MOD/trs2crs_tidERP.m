...
%   25 Jul 2012 by Sigrid Boehm: Implementation of 
%       partial set-up for high-frequency tidal ERP variations. 
% *************************************************************************
function [t2c,dQdx,dQdy,dQdut,dQdX,dQdY,X,Y,era,...
          PNn,Rn,Wn,R3ss,R2xp,dR2xp,R1yp,dR1yp,dRdutn,...         
          cosag1,sinag1,cosag2,sinag2] = ...
                                trs2crs_tidERP(mjd,xp,yp,dut1,dX,dY,nutmod)
...
% saved for estimation of tidal ERP variations
PNn   = zeros(3,3,n);
Rn    = zeros(3,3,n);
Wn    = zeros(3,3,n);
R3ss  = zeros(3,3,n);
...
% tidal ERP terms
[cosag1, sinag1, cosag2, sinag2] = par_tiderp(tt);
...
%     dPNdY 
    ...
    R3ss(:,:,i)   = rotm(-ss(i),3);
    R2xp(:,:,i)   = rotm(xp(i),2);
    dR2xp(:,:,i)  = drotm(xp(i),2);
    R1yp(:,:,i)   = rotm(yp(i),1);
    dR1yp(:,:,i)  = drotm(yp(i),1);
    dRdutn(:,:,i) = dRdut;
