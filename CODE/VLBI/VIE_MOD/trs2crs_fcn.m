...
%   11 Jun 2016 by Hana Krasna: implementation of partial set-up for the
%   FCN period and amplitudes
% *************************************************************************
function [t2c,dQdx,dQdy,dQdut,dQdX,dQdY,X,Y,era,dQdfcn,dQdfcnAc,dQdfcnAs] = ...
                                      trs2crs_fcn(mjd,xp,yp,dut1,dX,dY,nutmod)
...
dQdfcn    =zeros(3,3,n);
dQdfcnAc  =zeros(3,3,n);
dQdfcnAs  =zeros(3,3,n);
...
%              rotm(-E,3)* rotm(-d,2)* rotm(E,3)*drotm(S(i),3)*  dSdY;
    ...
    [Qfcn, dQfcn, dQfcnAc, dQfcnAs] = fcnnut(tt(i)); %hana
    PN    = Qfcn*Mcio*rotm(S(i),3); %hana
...
%                0                , -1                 , -Y(i)-X(i)^2*Y(i)/4];
    ...
    dMciodX = [-X(i) - X(i)^3/2 - (X(i)*Y(i)^2/4)         , -Y(i)/2 - 3*X(i)^2*Y(i)/8 - Y(i)^3/8,    1;...
               -Y(i)/2 - 3*X(i)^2*Y(i)/8 - Y(i)^3/8,        -X(i)*Y(i)^2/4                      ,   0;...
               -1                                       , 0                      , -X(i)-X(i)^3/2-X(i)*Y(i)^2/2]; %hana
           
    dMciodY = [-X(i)^2*Y(i)/4   ,                           -X(i)/2 - X(i)^3/8 - 3*X(i)*Y(i)^2/8   , 0;...
               -X(i)/2 - X(i)^3/8 - 3*X(i)*Y(i)^2/8  , -Y(i) - Y(i)^3/2 - (Y(i)*X(i)^2/4)        , 1;...
               0                , -1                 , -Y(i)-Y(i)^3/2-Y(i)*X(i)^2/2]; %hana

%     dPNdXneu= dMciodX * rotm(S(i),3) + Mcio*drotm(S(i),3)*dSdX
%     dPNdYneu= dMciodY * rotm(S(i),3) + Mcio*drotm(S(i),3)*dSdY
    dPNdX= Qfcn*dMciodX * rotm(S(i),3) + Qfcn*Mcio*drotm(S(i),3)*dSdX; %hana
    dPNdY= Qfcn*dMciodY * rotm(S(i),3) + Qfcn*Mcio*drotm(S(i),3)*dSdY; %hana
    
    dPNdfcn=dQfcn*Mcio*rotm(S(i),3); %hana
    dPNdfcnAc=dQfcnAc*Mcio*rotm(S(i),3); 
    dPNdfcnAs=dQfcnAs*Mcio*rotm(S(i),3); 
    ...
    dQdfcn(:,:,i)=dPNdfcn*     R * W; %[day]
    dQdfcnAc(:,:,i)=dPNdfcnAc*     R * W; %[-]
    dQdfcnAs(:,:,i)=dPNdfcnAs*     R * W; %[-]
