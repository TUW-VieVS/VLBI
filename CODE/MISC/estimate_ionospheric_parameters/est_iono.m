...
%
%   Reference: Hobiger, 2006: VLBI as a tool to probe the ionosphere
%
%   Coded for VieVS: 
...
%   gradient: North-South gradient, usage: (1 + dlat*gradient) [1/rad]
...
i_unkn = [1 1+3*(1:n_st-1)+cumsum(n_int(1:end-1))]; %index of first unknown per station
...
B = zeros(n_st+sum(n_int),3*n_st+sum(n_int));
for i = 1:n_st
   B(i_unkn(i)-(i-1)*2:i_unkn(i)-(i-1)*2+n_int(i), i_unkn(i)+2) = ones(n_int(i)+1,1);
   B(i_unkn(i)-(i-1)*2+1:i_unkn(i)-(i-1)*2+n_int(i), i_unkn(i)+3:i_unkn(i)+3+n_int(i)-1) = ...
...
A = zeros(n+1,3*n_st+sum(n_int));
A(end,i_unkn)=ones(1,n_st); %datum of instrumental biases: their sum is zero
x0 = zeros(3*n_st+sum(n_int),1); %initial guess for unknowns, used for L0, A
x0(i_unkn+1) = 1;
x0(i_unkn+2) = 20;
...
for k=1:max_iter %iterative adjustment
    ...
    %L0
    for i = 1:n
        dt_times_rt1(i)=int_diffs1(i,1:n_int(i1(i)))*x0(i_unkn(i1(i))+3:i_unkn(i1(i))+n_int(i1(i))+2);
        dt_times_rt2(i)=int_diffs2(i,1:n_int(i2(i)))*x0(i_unkn(i2(i))+3:i_unkn(i2(i))+n_int(i2(i))+2);
    end
    L0 = factor*(-m1.*(1 + x0(i_unkn(i1)+1).*dlat1).*...
        (x0(i_unkn(i1)+2) + dt_times_rt1) + m2.*(1 + x0(i_unkn(i2)+1).*dlat2).*...
        (x0(i_unkn(i2)+2) + dt_times_rt2)) - x0(i_unkn(i1)) + x0(i_unkn(i2));
    ...
    %Design matrix A
    for i = 1:n
       A(i,i_unkn(i1(i)):i_unkn(i1(i))+n_int(i1(i))+2) = -1*[1 factor*m1(i)*dlat1(i)*...
           (x0(i_unkn(i1(i))+2) + dt_times_rt1(i)) factor*m1(i)*(1+x0(i_unkn(i1(i))+1)*dlat1(i))...
           factor*m1(i)*(1+x0(i_unkn(i1(i))+1)*dlat1(i))*int_diffs1(i,1:n_int(i1(i)))];
       
       A(i,i_unkn(i2(i)):i_unkn(i2(i))+n_int(i2(i))+2) = [1 factor*m2(i)*dlat2(i)*...
           (x0(i_unkn(i2(i))+2) + dt_times_rt2(i)) factor*m2(i)*(1+x0(i_unkn(i2(i))+1)*dlat2(i))...
           factor*m2(i)*(1+x0(i_unkn(i2(i))+1)*dlat2(i))*int_diffs2(i,1:n_int(i2(i)))];
    end
...
ion = struct('st_name',{},'mjd',{},'vtec',{},'vtec_sd',{},'instr_bias',{},...
    'instr_bias_sd',{},'gradient',{},'gradient_sd',{});
...
for i=1:n_st
    ...
    var = Qxxd(i_unkn(i):i_unkn(i)+2+n_int(i))*s0^2; %variance of unknowns
    %populate ion struct
    ion(i).st_name = antenna(i).name;
    ion(i).mjd = t_int(1:n_int(i)+1,i)/24 + floor(mjd(1));
    ion(i).vtec = B(ind,:)*x0;
    ion(i).instr_bias = x0(i_unkn(i));
    ion(i).gradient = x0(i_unkn(i)+1);
    ion(i).instr_bias_sd = sqrt(var(1));
    ion(i).gradient_sd = sqrt(var(2));
    %error propagation (from offs+rt to vtec):
    ion(i).vtec_sd = sqrt(B(ind,i_unkn(i):i_unkn(i)+2+n_int(i)).^2*var);
end
