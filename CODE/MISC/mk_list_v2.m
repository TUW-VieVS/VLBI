...
% PRELIMINARY VERSION - NEEDS TO BE TUNED PRECISELY
% gives a processlist of all sessions from the masterfile v2 which you have in DATA/vgosDB
function [list,names]=mk_list_v2(varargin)


%[list,names]=mk_list_v2('ALL','VGOSDB','YEARS',2023,'NOINT')
    ...
    for session_type = 0:2      % 0 = 24h SX, 1 = INT, 2 = VGOS
        switch session_type
            case 0 % S/X 24h
                ...
                path_master_file = sprintf('../DATA/MASTER/master%04d.txt',year);
            
            case 1 % INT
                if no_intensives==1
                    continue
                end
                path_master_file = sprintf('../DATA/MASTER/master%04d-int.txt',year);
            
            case 2 % VGOS
                if no_vgos==1
                    continue
                end
                path_master_file = sprintf('../DATA/MASTER/master%04d-vgos.txt', year')
        end
        
        if exist(path_master_file,'file')
            

            [sesname, sescode, sestype, sesdate, sesstat] = read_master_v2(year);
...
%                         code = '__';
%                     end
%                     sess = sprintf('%02d%s%s',date,code);
%                     
%                     a = 1;
